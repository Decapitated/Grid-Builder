...
public class GridBuilder : MonoBehaviour
{
    ...
    // Covert polygon into quads in an A = center; C = polygon point; orientaion.
    List<Quad> PolygonToQuads(Polygon polygon)
    {
        var quads = new List<Quad>();

        var numPoints = polygon.Points.Count;
        var a = polygon.Center;
        for(int i = 0; i < numPoints; i++)
        {
            var b = polygon.Points[(i - 1) % numPoints];
            var c = polygon.Points[i];
            var d = polygon.Points[(i + 1) % numPoints];
            quads.Add(new(
                new(a, b, c),
                new(c, d, a)));
        }
    ...
    int NumSharedPolygonPoint(Polygon polygon, Vector2 point)
    {
        var pointEdges = polygon.VertexToEdges[point];
        var numShapes = 0;
        foreach(var edge in pointEdges)
        {
            List<Vector2> sharedShapes;
            if (dualGraph.EdgeToShapesVertex.TryGetValue(edge, out sharedShapes))
                //ALERT: Check if the shapes are used.
                numShapes += sharedShapes.Count - 1;
